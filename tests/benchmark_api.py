...
log_file = logging.FileHandler(
    filename="benchmarking_dummydata.log", mode="a", encoding="utf-8"
)
...
if __name__ == "__main__":
    ...
    factory = APIFactory()
    cdb_api = factory.construct_DB_API(
    ...
    logger.info(
        msg="add One detector to the database%15.6f%13.6f"
    ...
    cpu_time_end = time.clock()
    logger.info(
        msg="add 100 detectors to the database%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
    ...
    cpu_time_end = time.clock()
    logger.info(
        msg="List detectors%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
    ...
    dummydata_generator.create_multilevel_detectors(
        5, 5, "detector", None, group_detector_parent
    )
    ...
    for detector_and_parent in group_detector_parent:
        ...
        cdb_api.add_condition(
            reconstructed_detector_id, "daniel", "tag1", values, "calibration"
        )

    wall_time_end = time.time()
    cpu_time_end = time.clock()
    logger.info(
        msg="create Complex Structure%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
    ...
    cpu_time_end = time.clock()
    logger.info(
        msg="add One massive Condition to the deepest level%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
    ...
    cpu_time_end = time.clock()
    logger.info(
        msg="get a massive Condition by name from the deepest level%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
    ...
    cpu_time_end = time.clock()
    logger.info(
        msg="get Conditions by tag from the deepest level%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
    ...
    cpu_time_end = time.clock()
    logger.info(
        msg="update a Condition at the deepest level%15.6f%13.6f"
        % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start)
    )
