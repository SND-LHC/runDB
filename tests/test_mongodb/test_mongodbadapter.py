    ...
    if has_correct_parameter:
        ...
        if isinstance(result, dict):
            ...
            if detector_id == "/detector_id_exist/":
                assert (
                    result["name"] == detector_id.split("/")[1]
            ...
            if result["name"] == "detector_id_exist":
                assert (
                    result["conditions"][0]["name"] == "condition_exist"
                ), "condition name has wrong value"
                assert (
                    result["conditions"][0]["tag"] == "tag"
                ), "condition tag has wrong value"
                assert (
                    result["conditions"][0]["type"] == "type"
                ), "condition tag has wrong value"
                assert (
                    result["conditions"][0]["collected_at"]
                    == "2020,03,12,11,05,27,000000"
                ), "condition collected_at has wrong value"
                assert (
                    result["conditions"][0]["valid_since"]
                    == "2020,03,12,11,05,27,000000"
                ), "condition valid_since has wrong value"
                assert (
                    result["conditions"][0]["valid_until"]
            ...
            if result["name"] == "sub_detector_id_exist":
                assert (
                    result["conditions"][0]["name"] == "sub_detector_id_exist"
                ), "condition name has wrong value"
                assert (
                    result["conditions"][0]["tag"] == "sub_tag"
                ), "condition tag has wrong value"
                assert (
                    result["conditions"][0]["type"] == "sub_type"
                ), "condition tag has wrong value"
                assert (
                    result["conditions"][0]["collected_at"]
                    == "2020,04,12,11,05,27,000000"
                ), "condition collected_at has wrong value"
                assert (
                    result["conditions"][0]["valid_since"]
                    == "2020,04,12,11,05,27,000000"
            ...
            if result["name"] == "sub_sub_detector_id_exist":
                assert (
                    result["conditions"][0]["name"] == "sub_sub_detector_id_exist"
                ), "condition name has wrong value"
                assert (
                    result["conditions"][0]["tag"] == "sub_sub_tag"
                ), "condition tag has wrong value"
                assert (
                    result["conditions"][0]["type"] == "sub_sub_type"
...
def test_get_conditions(cdb_api, detector_id):
    ...
    if has_correct_parameter:
        if detector_id == "detector_without_condition":
            assert (
                cdb_api.get_conditions(detector_id) is None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "detector_id_exist",
            "condition_exist",
            datetime.datetime(2020, 3, 16, 12, 7, 30),
            None,
        ),
        ...
        (
            "detector_id_exist",
            "condition_exist",
            datetime.datetime(2020, 3, 12, 11, 5, 27, 555555),
            None,
        ),
...
def test_get_conditions_by_name_and_validity(
    ...
    if not isinstance(detector_id, str):
        ...
        # raise TypeError when detector_id is not an str.
        with pytest.raises(TypeError):
            assert cdb_api.get_conditions_by_name_and_validity(
                detector_id, name, start_date, end_date
            )
    ...
    if has_correct_parameter_type:
        result = cdb_api.get_conditions_by_name_and_validity(
            detector_id, name, start_date, end_date
        )
        # check if the result is a list or None
        assert (
        ...
        valid_cases = [
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == datetime.datetime(2020, 3, 16, 12, 7, 30),
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == "2020-03-16 12:07:30",
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == "2020-03-16 12:07:30:129920",
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == datetime.datetime(2020, 3, 12, 11, 5, 27, 767563),
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == datetime.datetime(2020, 3, 12, 11, 5, 27, 555555),
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == datetime.datetime(2020, 3, 13),
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == "2020-03-16 12:07:30",
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == "2020-03-16 12:07",
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == "2020-03-16 12",
            detector_id == "detector_id_exist"
            and name == "condition_exist"
            and start_date == "2020-03-16",
        ]
        invalid_cases = [
            detector_id == "detector_id_exist"
            and name == "condition_exist"
        ...
        if True in valid_cases:
            ...
            assert (
                result[0]["collected_at"] == "2020,03,12,11,05,27,000000"
            ), "condition collected_at value is wrong"
            assert (
                result[0]["valid_since"] == "2020,03,12,11,05,27,000000"
            ), "condition valid_since value is wrong"
            assert (
                result[0]["valid_until"] == "2020,04,10,11,05,27,000000"
...
def test_get_condition_by_name_and_tag(cdb_api, detector_id, name, tag):
    ...
    if has_correct_parameter_type:
        ...
        if (
            ...
            assert (
                result["collected_at"] == "2020,03,12,11,05,27,000000"
            ), "condition collected_at value is wrong"
            assert (
                result["valid_since"] == "2020,03,12,11,05,27,000000"
            ), "condition valid_since value is wrong"
            assert (
                result["valid_until"] == "2020,04,10,11,05,27,000000"
            ), "condition valid_until value is wrong"
...
def test_get_condition_by_name_and_collection_date(
    ...
    if not isinstance(detector_id, str):
        ...
        # raise TypeError when detector_id is not an str.
        with pytest.raises(TypeError):
            assert cdb_api.get_condition_by_name_and_collection_date(
                detector_id, name, collected_at
            )
    ...
    if has_correct_parameter_type:
        result = cdb_api.get_condition_by_name_and_collection_date(
            detector_id, name, collected_at
        )
    ...
    if has_correct_parameter:
        ...
        if parent_id is None:
            assert sorted(result) == sorted(
                ["detector_id_exist", "detector_without_condition"]
...
def test_add_condition(
    ...
    if (
        ...
        if (
            not has_correct_format(collected_at)
            or not has_correct_format(valid_until)
            or not has_correct_format(valid_since)
        ):
            has_correct_parameter_type = False
            with pytest.raises(ValueError):
                assert cdb_api.add_condition(
...
def test_update_condition_by_name_and_tag(
    ...
    if (
        ...
        with pytest.raises(TypeError):
            assert cdb_api.update_condition_by_name_and_tag(
                detector_id, name, tag, test_type, valid_since, valid_until
            )
